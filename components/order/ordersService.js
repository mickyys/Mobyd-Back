...
api.get('/:status/:datestart/:dateend', [auth], asyncMiddleware(async(req,res) =>{
    let result = await getOrders(req.params.status, req.params.datestart, req.params.dateend);
    res.status(200).send({
        result
    });
}));
...
api.put('/payment',  auth, asyncMiddleware(async(req,res) =>{
    let result = await updPayment(req.body._id, req.body.order.status, req.body.payment);
    res.status(200).send({
        result
    });
}));
...
api.put('/paymentAll',  auth, asyncMiddleware(async(req,res) =>{
    let result = await updPaymentAll(req.body._id, req.body.order.status, req.body.payment, req.body.products, req.body.services);
    res.status(200).send({
        result
    });
}));
